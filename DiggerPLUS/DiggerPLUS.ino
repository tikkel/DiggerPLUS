...
byte curlevel = 0;
byte maxlevel = 0;
byte maxhard = 0;  //maximal geschafftes Level im HARD-Mode
byte curhard = 0;
byte maxeasy = 0;  //maximal geschafftes Level im EASY-Mode
byte cureasy = 0;
byte lives = 0;
byte liveseasy = 0;
byte liveshard = 0;
...
void initWorld(byte _level) {
  gb.lights.clear();
  zfg = (gamemode == EASY) ? INDEX_WHITE : INDEX_WHITE;
  zbg = (gamemode == EASY) ? INDEX_BLACK : INDEX_RED;
  countdown = 5000;
...
void loadGame() {
  ...
  SerialUSB.printf("       maxeasy:%i maxhard:%i\n", maxeasy, maxhard);
