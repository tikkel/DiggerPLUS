...
void updateCursor() {
  ...
  if (camera_x < camera_x_target - SPRITE_W * 3) {  //scrolling ON
    scroll_xp = true;
    scroll_xm = false;
  } else if (camera_x == camera_x_target)  //scrollOff, wenn zentriert
    scroll_xp = false;
  if (scroll_xp) {
    camera_x = camera_x + 1;  //scrolle 1 pixel
  ...
  // nach unten scrollen
  if (camera_y < camera_y_target - SPRITE_H * 3) {  //scrolling ON
    scroll_yp = true;
    scroll_ym = false;
  } else if (camera_y == camera_y_target)  //scrolling OFF, wenn zentriert
    scroll_yp = false;
  if (scroll_yp) {
    camera_y = camera_y + 1;  //scrolle 1 pixel
  ...
  if (scroll_ym) {
    camera_y = camera_y - 1;         //scrolle 1 pixel
    if (camera_y < -TOPBAR_H + 1) {  //TOPBAR_H ist gleich die Menüzeilenhöhe
      camera_y = -TOPBAR_H + 1;
      scroll_ym = false;
    }
  }
